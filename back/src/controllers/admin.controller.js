...
exports.getDashboardStats = async (req, res) => {
  try {
    ...
    // Calculate total requestCount from all technician profiles
    const profiles = await TechnicianProfile.find({});
    const totalRequests = profiles.reduce((acc, p) => acc + (p.requestCount || 0), 0);
...
exports.resetPassword = async (req, res) => {
  try {
    ...
    // Optional: Force user to change password on next login if they are a technician
    if (user.role === 'technician') {
