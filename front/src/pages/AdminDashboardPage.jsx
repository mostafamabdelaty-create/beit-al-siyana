...
function AdminDashboardPage() {
  ...
  const loadStats = async () => {
    try {
      const res = await fetch(toApiUrl('/admin/dashboard'), { headers: authHeaders() });
  ...
  const loadRequests = async () => {
    try {
      const res = await fetch(toApiUrl('/join/admin'), { headers: authHeaders() });
      const result = await res.json();
  ...
  const loadCustomers = async () => {
    try {
      const res = await fetch(toApiUrl('/admin/customers'), { headers: authHeaders() });
      const result = await res.json();
      if (result.success && result.data) {
  ...
  const loadTechnicians = async () => {
    try {
      const res = await fetch(toApiUrl('/admin/technicians-with-plans'), { headers: authHeaders() });
      const result = await res.json();
      if (result.success && result.data) {
  ...
  const loadPackages = async () => {
    ...
    try {
      const res = await fetch(toApiUrl('/packages?includeInactive=1'));
      const result = await res.json();
  ...
  const toggleUser = async (id, action) => {
    try {
      const res = await fetch(toApiUrl(`/admin/users/${id}/${action}`), {
        method: 'PUT',
        headers: authHeaders()
      });
      if (res.ok) {
  ...
  const deleteUser = async (id) => {
    ...
    try {
      const res = await fetch(toApiUrl(`/admin/users/${id}`), {
        method: 'DELETE',
        headers: authHeaders()
      });
      if (res.ok) {
  ...
  const resetPassword = async (id) => {
    ...
    try {
      const res = await fetch(toApiUrl(`/admin/users/${id}/reset-password`), {
        method: 'PUT',
  ...
  const approveRequest = async (id, type) => {
    ...
    try {
      const res = await fetch(toApiUrl(`/join/admin/${id}/approve`), {
        method: 'PUT',
        headers: authHeaders()
      });
      const result = await res.json();
  ...
  const rejectRequest = async (id) => {
    ...
    try {
      const res = await fetch(toApiUrl(`/join/admin/${id}/reject`), {
        method: 'PUT',
        headers: authHeaders(true),
  ...
  const savePlanForm = async () => {
    ...
    try {
      const res = await fetch(toApiUrl(`/packages${isEdit ? `/${planForm.id}` : ''}`), {
  ...
  const togglePlanStatus = async (planId, shouldActivate) => {
    ...
    try {
      const res = await fetch(toApiUrl(`/packages/${planId}`), {
        method: 'PUT',
        headers: authHeaders(true),
                      ...
                      <tr key={req._id}>
                        ...
                        <td>
                          {(req.paymentScreenshot || req.screenshot) ? (
                            <button className="action-btn btn-primary" onClick={() => setImageModalUrl(toMediaUrl((req.paymentScreenshot || req.screenshot).replace(/\\/g, '/')))} style={{ padding: '4px 8px', fontSize: '0.75rem' }}>
